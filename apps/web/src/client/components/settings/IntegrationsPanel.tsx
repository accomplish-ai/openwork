...
const PLATFORMS: {
...
}[] = [
  {
    id: 'whatsapp',
    name: 'WhatsApp',
    description: 'Trigger tasks and receive updates via WhatsApp messages',
    icon: (
  ...
  },
  {
    id: 'slack',
    name: 'Slack',
    description: 'Connect Slack workspace for task management',
    icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    id: 'telegram',
    name: 'Telegram',
    description: 'Use Telegram bot to interact with Accomplish',
    icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
  ...
  },
  {
    ...
    description: 'Integrate with Teams channels and chats',
    icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-card p-5">
        <div className="font-medium text-foreground">Messaging Integrations</div>
        <p className="mt-1.5 text-sm text-muted-foreground leading-relaxed">
          Connect messaging platforms to trigger Accomplish tasks remotely. When enabled with tunnel
          access, you can start and monitor tasks from your phone or any device.
        </p>
      </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">
                      Allow receiving and responding to messages
                    </p>
                  </div>
                  <button
                    role="switch"
                ...
                {platform.id === 'whatsapp' && isEnabled && (
                  <div className="rounded-lg border border-border p-4">
                    ...
                    <p className="text-xs text-muted-foreground mb-3">
                      Scan the QR code below with your WhatsApp app to connect this device. Go to
                        ...
                        <div className="h-48 w-48 rounded-lg bg-muted flex items-center justify-center mx-auto">
                          <div className="text-center text-muted-foreground">
                            ...
                            <span className="text-xs">
                              QR code will appear here when the WhatsApp service starts
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                {isEnabled && (
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">
                        Open a secure tunnel so messages can reach this machine from anywhere
                      </p>
                    </div>
                    <button
                      role="switch"
                      ...
                      disabled={saving}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200 ${
                        integration?.tunnelEnabled ? 'bg-primary' : 'bg-muted'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ${
                          integration?.tunnelEnabled ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
