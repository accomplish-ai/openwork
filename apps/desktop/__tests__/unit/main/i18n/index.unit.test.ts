 ...
 */
function mockMultipleLocaleFiles(
  files: Array<{ lang: string; ns: string; data: Record<string, unknown> }>,
) {
...
describe('Main Process i18n', () => {
  ...
  describe('t() — namespace parsing', () => {
    it('should default to common namespace', () => {
      mockMultipleLocaleFiles([{ lang: 'en', ns: 'common', data: { greeting: 'Hello' } }]);
      initializeI18n('en');
    ...
    it('should return raw key for unrecognized namespace prefix', () => {
      mockMultipleLocaleFiles([{ lang: 'en', ns: 'common', data: { key: 'from common' } }]);
      initializeI18n('en');
  ...
  describe('t() — English fallback', () => {
    ...
    it('should not attempt redundant fallback when already English', () => {
      mockMultipleLocaleFiles([{ lang: 'en', ns: 'common', data: {} }]);
      initializeI18n('en');
  ...
  describe('caching', () => {
    it('should read locale file from disk only once', () => {
      mockMultipleLocaleFiles([{ lang: 'en', ns: 'common', data: { key: 'value' } }]);
      initializeI18n('en');

      expect(t('key')).toBe('value');
      expect(t('key')).toBe('value');

      const commonCalls = mockReadFileSync.mock.calls.filter((call) =>
        (call[0] as string).includes('common.json'),
      );
  ...
  describe('getTranslations', () => {
    it('should return full JSON object for a namespace', () => {
      mockMultipleLocaleFiles([{ lang: 'en', ns: 'settings', data: { title: 'Settings' } }]);
