...
const mockSetI18nLanguage = vi.fn((lang: string) => {
...
describe('i18n IPC Handlers', () => {
  ...
  describe('i18n:set-language', () => {
    ...
    it('should reject unsupported language', async () => {
      await expect(invokeHandler('i18n:set-language', 'fr-FR')).rejects.toThrow(
        'errors:i18n.unsupportedLanguage',
      );
    });
  ...
  describe('i18n:get-translations', () => {
    it('should return translations for specified language', async () => {
      const result = (await invokeHandler('i18n:get-translations', 'en')) as {
        language: string;
    ...
    it('should fall back to current language for unsupported param', async () => {
      mockI18nLanguage = 'en';
      const result = (await invokeHandler('i18n:get-translations', 'invalid')) as {
        language: string;
      };
      expect(result.language).toBe('en');
    });
  ...
  describe('initialization', () => {
    it('should not initialize i18n during handler registration (initialized in index.ts)', () => {
      mockHandlers.clear();
      vi.clearAllMocks();
      registerIPCHandlers();
