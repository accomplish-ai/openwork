...
const accomplishAPI = {
  ...
  onHuggingFaceDownloadProgress: (callback: (progress: { modelId: string; status: string; progress: number; error?: string }) => void) => {
    const listener = (_: unknown, progress: { modelId: string; status: string; progress: number; error?: string }) => callback(progress);
