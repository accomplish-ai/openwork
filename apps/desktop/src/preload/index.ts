...
const accomplishAPI = {
  ...
  getMessagingConfig: (): Promise<unknown> => ipcRenderer.invoke('settings:messaging-config:get'),
  setMessagingConfig: (config: unknown): Promise<void> =>
    ipcRenderer.invoke('settings:messaging-config:set', config ? JSON.stringify(config) : null),
