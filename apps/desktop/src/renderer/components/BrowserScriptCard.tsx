...
function formatActionLabel(action: BrowserAction, t: (key: string, options?: Record<string, unknown>) => string): string {
  const maxLength = 25;
  let label = '';
  ...
  switch (action.action) {
    case 'goto': {
      try {
        ...
        label = t('browserActions.navigate', { hostname });
      } catch {
        label = t('browserActions.navigateDefault');
      }
      break;
    }
    case 'findAndFill':
    case 'fillByRef': {
      ...
      label = text ? t('browserActions.fill', { text }) : t('browserActions.fillDefault');
      break;
    }
    case 'findAndClick':
    case 'clickByRef': {
    ...
    case 'keyboard':
      label = t('browserActions.press', { key: action.key || 'key' });
      break;
    case 'snapshot':
      label = t('browserActions.capture');
      break;
    case 'screenshot':
      label = t('browserActions.screenshot');
      break;
    case 'waitForSelector':
      label = t('browserActions.waitElement');
      break;
    case 'waitForLoad':
      label = t('browserActions.waitPage');
      break;
    case 'waitForNavigation':
      label = t('browserActions.waitNavigation');
      break;
    case 'evaluate':
        ...
        {hasMore && (
          <>
            ...
            <button
              ...
              aria-label={expanded ? t('browserActions.showLess') : t('browserActions.showMore', { count: hiddenCount })}
              className={cn(
            ...
            >
              {expanded ? t('browserActions.showLess') : t('browserActions.showMore', { count: hiddenCount })}
            </button>
          </>
        )}
      </div>
