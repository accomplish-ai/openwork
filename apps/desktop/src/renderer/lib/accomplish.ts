...
interface AccomplishAPI {
  ...
  addApiKey(provider: 'anthropic' | 'openai' | 'openrouter' | 'google' | 'xai' | 'deepseek' | 'moonshot' | 'zai' | 'azure-foundry' | 'custom' | 'bedrock' | 'litellm' | 'minimax' | 'lmstudio' | 'vertex' | 'nebius' | 'together' | 'fireworks' | 'groq' | 'elevenlabs', key: string, label?: string): Promise<ApiKeyConfig>;
