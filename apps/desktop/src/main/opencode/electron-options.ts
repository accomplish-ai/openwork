...
export async function buildEnvironment(taskId: string): Promise<NodeJS.ProcessEnv> {
  ...
  const hfProvider = activeModel?.provider === 'huggingface-local' || selectedModel?.provider === 'huggingface-local';
  ...
  const envConfig: EnvironmentConfig = {
    ...
    openAiBaseUrl: hfBaseUrl || configuredOpenAiBaseUrl || undefined,
    ollamaHost,
  };
