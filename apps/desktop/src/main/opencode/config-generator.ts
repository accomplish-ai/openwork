...
**STEP 3: UPDATE TODOS IMMEDIATELY**
- Mark a todo "in_progress" when you start working on it
- Mark it "completed" **immediately upon finishing** - don't batch updates
- Keep only ONE todo as "in_progress" at a time
- When all work is done, all todos should be "completed" or "cancelled"

WRONG: Work on multiple steps, then update all todos at once at the end
CORRECT: Finish step → immediately call todowrite → start next step
