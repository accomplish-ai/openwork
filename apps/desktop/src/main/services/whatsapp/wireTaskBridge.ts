...
export function wireTaskBridge(service: WhatsAppService): { bridge: TaskBridge } {
  ...
  const bridge = new TaskBridge(service, async (senderId, senderName, text) => {
    ...
    service.sendMessage(senderId, `⏳ Task started: "${text.slice(0, 80)}${text.length > 80 ? '…' : ''}"`).catch(() => {});
    ...
    try {
      ...
      await taskManager.startTask(taskId, {
        ...
        onBatchedMessages: (messages: TaskMessage[]) => {
          ...
          if (lastAssistantContent && now - lastProgressSentAt >= PROGRESS_RATE_LIMIT_MS) {
            ...
            const preview = lastAssistantContent.length > 200
        ...
        onProgress: () => {},
        onPermissionRequest: () => {
          service.sendMessage(senderId, 'Task requires a permission that cannot be auto-approved. It has been denied for safety.').catch(() => {});
          taskManager.sendResponse(taskId, 'no').catch(() => {});
        },
        onComplete: (result) => {
        ...
        onError: () => {
          service.sendMessage(senderId, 'Sorry, the task encountered an error. Please try again.').catch(() => {});
          bridge.clearActiveTask(senderId);
        },
        onDebug: () => {},
        onStatusChange: () => {},
      ...
      await service.sendMessage(senderId, 'Sorry, I could not process your request.').catch(() => {});
      bridge.clearActiveTask(senderId);
    }
  });

  const existingConfig = storage.getMessagingConfig('whatsapp');
  if (existingConfig?.ownerJid) {
    bridge.setOwnerJid(existingConfig.ownerJid);
  }
  if (existingConfig?.ownerLid) {
    bridge.setOwnerLid(existingConfig.ownerLid);
  }

  return { bridge };
}

export function wireStatusListeners(service: WhatsAppService, bridge: TaskBridge): void {
  const storage = getStorage();
