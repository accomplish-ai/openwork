 ...
 */
export type ApiKeyProvider = 'anthropic' | 'openai' | 'openrouter' | 'google' | 'groq' | 'custom';

/**
 * Get all API keys for all providers
 */
export async function getAllApiKeys(): Promise<Record<ApiKeyProvider, string | null>> {
  const [anthropic, openai, openrouter, google, groq, custom] = await Promise.all([
    getApiKey('anthropic'),
    getApiKey('openai'),
  ...
  return { anthropic, openai, openrouter, google, groq, custom };
}
