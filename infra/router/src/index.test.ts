...
describe('resolveRoute with ?pin=', () => {
  it('routes to pinned version when valid and in activeVersions', () => {
    const route = resolveRoute(makeConfig(), makeUrl({ pin: PINNED_BUILD }), null, true);
  ...
  it('returns null when pin is not in activeVersions', () => {
    const route = resolveRoute(makeConfig(), makeUrl({ pin: '9.9.9-999' }), null, true);
    expect(route).toBeNull();
  });

  it('returns null when pin has invalid format', () => {
    const route = resolveRoute(makeConfig(), makeUrl({ pin: 'bad-format' }), null, true);
    expect(route).toBeNull();
  });
  ...
  it('cookie fast-path takes precedence on non-navigation requests', () => {
    ...
    const route = resolveRoute(makeConfig(), makeUrl({ pin: PINNED_BUILD }), cookie, false);
    expect(route).toEqual({
