...
function resolveOverride(config: RoutingConfig, desktopVersion: string | null): string | null {
...
export function isNavigation(request: Request, url: URL): boolean {
  if (url.searchParams.has('build') || url.searchParams.has('type') || url.searchParams.has('pin'))
    return true;
  if (url.pathname === '/') return true;
  if (!url.pathname.includes('.') && request.headers.get('accept')?.includes('text/html'))
    return true;
  return false;
}
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const config = env.ROUTING_CONFIG
      ? await env.ROUTING_CONFIG.get<RoutingConfig>(env.KV_CONFIG_KEY || 'config', {
          type: 'json',
          cacheTtl: 60,
        })
      : null;
    ...
    if (!config) {
      ...
      const fallbackWorker = (env as Record<string, Fetcher | undefined>)[
        `APP_${tier.toUpperCase()}`
      ];
      if (fallbackWorker) {
        console.log(
          JSON.stringify({
    ...
    const worker = (env as Record<string, Fetcher | undefined>)[
      bindingName(route.buildId, route.tier)
    ];
    ...
    const needsCookie =
      navigation || !cookie || cookie.buildId !== route.buildId || cookie.tier !== route.tier;
