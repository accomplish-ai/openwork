...
function makeRequest(path: string, method = "GET", body?: unknown, headers?: Record<string, string>): Request {
...
describe("audit cursor pagination", () => {
  ...
  it("returns cursor when list is not complete", async () => {
    (env.ROUTING_CONFIG.list as ReturnType<typeof vi.fn>).mockResolvedValue({
      keys: [{ name: "audit:0000:id1", metadata: { id: "id1", timestamp: "2026-01-01T00:00:00Z", action: "config_updated", source: "dashboard" } }],
      list_complete: false,
...
describe("rollback — multi-step previousDefault", () => {
  ...
  it("previousDefault updates to latest previous on successive changes", async () => {
    ...
    kvStore.set("config", {
      value: JSON.stringify({ default: "0.1.0-1", overrides: [], activeVersions: ["0.1.0-1", "0.2.0-2", "0.3.0-3"] }),
    });
...
describe("error handler", () => {
  ...
  it("returns 500 when handler throws TypeError", async () => {
    ...
    (env.ROUTING_CONFIG.get as ReturnType<typeof vi.fn>).mockRejectedValue(new TypeError("Cannot read properties"));
