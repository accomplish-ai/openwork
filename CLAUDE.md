...
| `CLOUDFLARE_API_TOKEN`      | CI / infra scripts   | Wrangler auth for prod deploys                                                                                                                                                                                                                                  |
| `CLOUDFLARE_ACCOUNT_ID`     | infra scripts        | Prod account — required for KV operations                                                                                                                                                                                                                       |
| `STAGING_CLOUDFLARE_API_TOKEN` | CI (preview workflows) | Wrangler auth for staging/preview deploys                                                                                                                                                                                                                    |
| `STAGING_CLOUDFLARE_ACCOUNT_ID` | CI (preview workflows) | Staging Cloudflare account ID                                                                                                                                                                                                                               |
| `STAGING_CF_SUBDOMAIN`      | CI (preview workflows) | Workers subdomain on staging account for health checks                                                                                                                                                                                                         |
| `KV_NAMESPACE_ID`           | CI / infra scripts   | Cloudflare KV namespace ID for routing config. Auto-provisioned by `ensure_kv_namespace()` in `infra/lib.sh` — no longer required as a GitHub variable                                                                                                          |
...
- Staging Cloudflare account used for PR previews (secrets: `STAGING_CLOUDFLARE_API_TOKEN`, `STAGING_CLOUDFLARE_ACCOUNT_ID`, var: `STAGING_CF_SUBDOMAIN`)
